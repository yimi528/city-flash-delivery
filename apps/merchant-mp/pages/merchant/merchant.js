...
Page({
  ...
  loadDashboard() {
    ...
    this.ensureMerchantSession().then(() => api.getMerchantDashboard(app.globalData.merchantId)).then((dashboard) => {
  ...
  ensureMerchantSession() {
    ...
    return api.merchantLogin({ merchantId: app.globalData.merchantId }).then((session) => {
      if (app.setMerchantSession) app.setMerchantSession(session)
    })
  },
