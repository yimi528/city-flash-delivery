const DEFAULT_BASE_URL = 'http://192.168.2.105:3000/api'
...
function getServiceProgressText(service, status) {
  ...
  const isPassenger = ['拼车', '顺风车', '送客'].some((keyword) => name.indexOf(keyword) !== -1)
  if (status === '取货中') {
...
function buildNestPricePayload(payload) {
  ...
  return {
    ...
    linePriceMultiplier: firstNumber(source.linePriceMultiplier, cargoOptions.linePriceMultiplier, servicePricing.linePriceMultiplier, 1),
